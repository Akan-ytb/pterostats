...
module.exports = async function sendMessage({ client, cache, panel, uptime, nodes, servers, users }) {
        ...
        if (config.nodes_settings.details) {
            nodes.forEach((node, index) => {
                ...
                embed.addFields({
                    ...
                    value:
                        "```\n" +
                        (config.nodes_settings.host ? `Lien   : ${node.attributes.fqdn}\n` : "") +
                        `Mémoire (RAM) : ${convertUnits(node.attributes.allocated_resources.memory, node.attributes.memory, config.nodes_settings.unit)}\n` +
                        `Stockage   : ${convertUnits(node.attributes.allocated_resources.disk, node.attributes.disk, config.nodes_settings.unit)}` +
                        (node.attributes?.allocated_resources?.cpu ? `\nCPU    : ${node.attributes?.allocated_resources?.cpu || 0}%` : "") +
                        (config.nodes_settings.servers ? `\nServeurs: ${node.attributes.relationships.servers}${config.nodes_settings.allocations_as_max_servers ? ` / ${node.attributes.relationships.allocations}` : ""}` : "") +
                        (config.nodes_settings.uptime ? `\nEn ligne depuis : ${node.uptime ? uptimeFormatter(Date.now() - node.uptime) : "N/A"}` : "") +
                        "```"
                });
            });
    ...
    let panelEmbed = new EmbedBuilder()
        ...
        .addFields({
            ...
            value:
                "```\n" +
                (config.panel_settings.host ? `Lien   : ${new URL(process.env.PanelURL).host}\n` : "") +
                `Nodes  : ${nodes.length}\n` +
                (config.panel_settings.servers ? `Serveurs: ${servers || "Unknown"}\n` : "") +
                (config.panel_settings.users ? `Utilisateur  : ${users || "Unknown"}\n` : "") +
                (config.panel_settings.uptime ? `En ligne depuis : ${uptime ? uptimeFormatter(Date.now() - uptime) : "N/A"}\n` : "") +
                "```"
        });
